...
sudo pacman -S --noconfirm bspwm sxhkd picom polybar rofi feh nitrogen alacritty

# Fuentes para Polybar
echo "Instalando fuentes para Polybar..."
sudo pacman -S --noconfirm ttf-font-awesome ttf-jetbrains-mono
...
# Habilitar Bluetooth de manera segura
if pacman -Qs bluez > /dev/null; then
    echo "Configurando servicio Bluetooth..."
    sudo systemctl enable bluetooth.service
    sudo systemctl start bluetooth.service
else
    echo "Paquete Bluetooth no instalado. Omitiendo configuración de servicio."
fi
...
yay -S --noconfirm brave-bin archlinux-wallpaper
...
cp -r ~/Descargas/arch/dotfiles/bspwm/* ~/.config/bspwm/
cp -r ~/Descargas/arch/dotfiles/sxhkd/* ~/.config/sxhkd/
cp -r ~/Descargas/arch/dotfiles/picom/* ~/.config/picom/
cp -r ~/Descargas/arch/dotfiles/polybar/* ~/.config/polybar/
cp -r ~/Descargas/arch/dotfiles/rofi/* ~/.config/rofi/
cp -r ~/Descargas/arch/dotfiles/alacritty/* ~/.config/alacritty/
...
mkdir -p ~/Wallpapers
cp -r /usr/share/backgrounds/archlinux/* ~/Wallpapers/
# Establecer fondo por defecto
nitrogen --set-zoom-fill --save ~/Wallpapers/archlinux-simplyblack.png
...
~/.config/polybar/launch.sh &     # Iniciar Polybar
nitrogen --restore &              # Restaurar fondo de pantalla
...
if [ ! -x ~/.config/polybar/launch.sh ]; then
    echo "Error: script de lanzamiento de Polybar no encontrado o sin permisos de ejecución"
    exit 1
fi
